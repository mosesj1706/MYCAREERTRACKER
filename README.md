...
| **Free credentials** (on Learning) | For the plan's gap skills, a web search steered at recognised issuers (AWS Skill Builder, Databricks, dbt, Astronomer, Snowflake, Confluent, Kaggle, freeCodeCamp, Microsoft Learn, HackerRank) for courses and assessments that end in a badge or certificate. Each result is classified honestly: cost (free / free to audit / paid), what you receive, and whether the issuer is the vendor, a major platform, or unrecognised; anything not free-and-recognised is greyed with the reason rather than hidden. "Done" hands it to Add to profile as a course, where a credential alone is capped at *familiar*. |
